...
try:
    ...
    if settings_obj.external_hosts:
        db_hosts = [host.strip() for host in settings_obj.external_hosts.split(',') if host.strip()]
...
# Format: comma-separated list of origins, e.g. "https://example.com,http://localhost"
...
try:
    ...
    if settings_obj.external_csrf_origins:
        db_origins = [origin.strip() for origin in settings_obj.external_csrf_origins.split(',') if origin.strip()]

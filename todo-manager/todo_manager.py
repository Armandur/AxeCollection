...
class TodoManager:
        ...
        for line in lines:
            ...
            if main_item_match and current_section:
                ...
                current_item = TodoItem(
                    item_num, completed, text, [], current_section.name
                )
    ...
    def _parse_sub_item(self, line: str, parent_item: TodoItem) -> bool:
        ...
        for level in range(2, 6):  # Nivå 2-5
            indent = "    " * (
                level - 1
            )  # 4 spaces per nivå (minus 1 eftersom level börjar på 2)
            ...
            if match:
                ...
                expected_parts = (
                    level  # Nivå 2 = X.Y (2 delar), Nivå 3 = X.Y.Z (3 delar), etc.
                )
                ...
                parent_container = self._find_parent_for_level(
                    parent_item, number, level
                )
    ...
    def _find_parent_for_level(
        self, root_item: TodoItem, number: str, level: int
    ) -> Optional[List[TodoSubItem]]:
    ...
    def save(self):
        ...
        for section_idx, section in enumerate(self.sections, 1):
            ...
            for item_idx, item in enumerate(section.items, 1):
                ...
                global_num = (
                    sum(len(s.items) for s in self.sections[: section_idx - 1])
                    + item_idx
                )
    ...
    def add_item(self, text: str, section_name: str, completed: bool = False):
        ...
        new_item = TodoItem(
            0, completed, text, [], section_name
        )  # Nummer sätts vid save()
        section.items.append(new_item)

        # Beräkna vilket nummer uppgiften kommer att få
        section_idx = self.sections.index(section) + 1
        global_num = sum(len(s.items) for s in self.sections[: section_idx - 1]) + len(
            section.items
        )

        print(
            f"✅ Lade till '{text}' i sektion '{section_name}' som uppgift #{global_num}"
        )
        return global_num
    ...
    def add_sub_item(self, parent_number: str, text: str, completed: bool = False):
        ...
        if len(parent_parts) == 1:
            ...
            if root_item:
                parent_container = self._find_sub_item_container(
                    root_item, parent_number
                )
    ...
    def _find_sub_item_container(
        self, root_item: TodoItem, target_number: str
    ) -> Optional[List[TodoSubItem]]:
    ...
    def swap_items(self, item1_number: int, item2_number: int):
        ...
        if section1 == section2:
            ...
            section1.items[pos1], section1.items[pos2] = (
                section1.items[pos2],
                section1.items[pos1],
            )
        else:
    ...
    def move_to_position(self, item_number: int, target_position: int):
        ...
        if target_position < 1 or target_position > total_items:
            print(
                f"❌ Position {target_position} är ogiltig! Måste vara mellan 1 och {total_items}"
            )
            return False
    ...
    def move_sub_item(self, sub_number: str, new_parent_number: str):
        ...
        if len(new_parent_parts) == 1:
            ...
            if not root_item:
                ...
                return False
            new_parent_container = self._find_sub_item_container(
                root_item, new_parent_number
            )
    ...
    def _update_sub_item_hierarchy(
        self, sub_item: TodoSubItem, new_number: str, new_level: int
    ):
    ...
    def swap_sub_items(self, sub1_number: str, sub2_number: str):
        ...
        pos1 = next(
            (
                i
                for i, item in enumerate(parent1_container)
                if item.number == sub1_number
            ),
            None,
        )
        pos2 = next(
            (
                i
                for i, item in enumerate(parent2_container)
                if item.number == sub2_number
            ),
            None,
        )
        ...
        if parent1_container is parent2_container:
            # Samma förälder
            parent1_container[pos1], parent1_container[pos2] = (
                parent1_container[pos2],
                parent1_container[pos1],
            )
        else:
            # Olika föräldrar
    ...
    def promote_sub_item(self, sub_number: str):
        ...
        if len(parts) <= 2:
            print(
                f"❌ Underuppgift {sub_number} kan inte flyttas upp! Redan på nivå 2."
            )
            return False
    ...
    def demote_sub_item(self, sub_number: str, target_sibling: str):
        ...
        if len(parts) >= 5:
            print(
                f"❌ Underuppgift {sub_number} kan inte flyttas ner! Redan på nivå 5."
            )
            return False
    ...
    def _list_sub_items_recursive(
        self, sub_items: List[TodoSubItem], level: int, only_incomplete: bool
    ):
        ...
        for sub_item in sub_items:
            ...
            indent = (
                "  " * level
            )  # 2 spaces per nivå för visning (mer kompakt än save-format)
            ...
            if sub_item.sub_items and level < 5:
                self._list_sub_items_recursive(
                    sub_item.sub_items, level + 1, only_incomplete
                )
            ...
            if incomplete_in_section:
                ...
                for item in section.items:
                    if not item.completed:
                        ...
                        self._list_sub_items_recursive(
                            item.sub_items, 2, only_incomplete=True
                        )
    ...
    def add_multiple_items(
        self, texts: List[str], section_name: str, completed: bool = False
    ):
        ...
        for text in texts:
            ...
            section_idx = self.sections.index(section) + 1
            global_num = sum(
                len(s.items) for s in self.sections[: section_idx - 1]
            ) + len(section.items)
...
def main():
    ...
    add_parser.add_argument(
        "--completed", action="store_true", help="Markera som klar direkt"
    )

    # Add multiple command
    add_multiple_parser = subparsers.add_parser(
        "add-multiple", help="Lägg till flera uppgifter"
    )
    add_multiple_parser.add_argument(
        "texts", nargs="+", help="Uppgiftstexter (separerade med space)"
    )
    add_multiple_parser.add_argument("section", help="Sektionsnamn")
    add_multiple_parser.add_argument(
        "--completed", action="store_true", help="Markera alla som klara direkt"
    )
    ...
    complete_multiple_parser = subparsers.add_parser(
        "complete-multiple", help="Markera flera uppgifter som klara"
    )
    complete_multiple_parser.add_argument(
        "numbers",
        nargs="+",
        help="Uppgiftsnummer (separerade med space, t.ex. 80 80.1 80.2)",
    )

    # Uncomplete command
    uncomplete_parser = subparsers.add_parser(
        "uncomplete", help="Markera uppgift som ej klar"
    )
    uncomplete_parser.add_argument("number", type=int, help="Uppgiftsnummer")

    # Complete sub-item command
    complete_sub_parser = subparsers.add_parser(
        "complete-sub", help="Markera underuppgift som klar"
    )
    complete_sub_parser.add_argument(
        "number", help="Underuppgiftsnummer (t.ex. 42.1 eller 42.1.2)"
    )

    # Uncomplete sub-item command
    uncomplete_sub_parser = subparsers.add_parser(
        "uncomplete-sub", help="Markera underuppgift som ej klar"
    )
    uncomplete_sub_parser.add_argument(
        "number", help="Underuppgiftsnummer (t.ex. 42.1 eller 42.1.2)"
    )

    # Move command
    move_parser = subparsers.add_parser(
        "move", help="Flytta uppgift till annan sektion"
    )
    ...
    move_pos_parser = subparsers.add_parser(
        "move-to", help="Flytta uppgift till specifik position"
    )
    move_pos_parser.add_argument("number", type=int, help="Uppgiftsnummer")
    move_pos_parser.add_argument(
        "position", type=int, help="Målposition (globalt nummer)"
    )

    # Move sub-item command
    move_sub_parser = subparsers.add_parser(
        "move-sub", help="Flytta underuppgift till ny förälder"
    )
    move_sub_parser.add_argument(
        "sub_number", help="Underuppgiftsnummer (t.ex. 42.1.2)"
    )
    move_sub_parser.add_argument("new_parent", help="Ny förälder (t.ex. 43 eller 43.1)")

    # Swap sub-items command
    swap_sub_parser = subparsers.add_parser(
        "swap-sub", help="Byter plats på två underuppgifter"
    )
    ...
    promote_parser = subparsers.add_parser(
        "promote-sub", help="Flytta underuppgift upp en nivå"
    )
    ...
    demote_parser = subparsers.add_parser(
        "demote-sub", help="Flytta underuppgift ner en nivå"
    )
    demote_parser.add_argument("sub_number", help="Underuppgiftsnummer (t.ex. 42.1)")
    demote_parser.add_argument(
        "target_sibling", help="Målsyskon att flytta under (t.ex. 42.2)"
    )
    ...
    remove_multiple_parser = subparsers.add_parser(
        "remove-multiple", help="Ta bort flera uppgifter"
    )
    remove_multiple_parser.add_argument(
        "numbers", nargs="+", type=int, help="Uppgiftsnummer (separerade med space)"
    )

    # Remove section command
    remove_section_parser = subparsers.add_parser(
        "remove-section", help="Ta bort sektion"
    )
    ...
    list_parser.add_argument(
        "--incomplete", action="store_true", help="Visa bara ej slutförda uppgifter"
    )

    # List all incomplete command
    subparsers.add_parser(
        "all-incomplete", help="Lista alla oklarade uppgifter från alla sektioner"
    )

    # List all command
    subparsers.add_parser(
        "all", help="Lista alla uppgifter från alla sektioner (både klara och oklara)"
    )
    ...
    show_parser = subparsers.add_parser(
        "show", help="Visa detaljerad information om en uppgift"
    )

...
def axe_list(request):
    ...
    total_buy_shipping = (
        filtered_transactions.filter(type="KÖP").aggregate(total=Sum("shipping_cost"))[
            "total"
        ]
        or 0
    )
    total_sale_shipping = (
        filtered_transactions.filter(type="SÄLJ").aggregate(total=Sum("shipping_cost"))[
            "total"
        ]
        or 0
    )
    total_profit = total_sale_value - total_buy_value
    total_profit_with_shipping = (total_sale_value + total_sale_shipping) - (
        total_buy_value + total_buy_shipping
    )

    # Hitta sålda yxor bland filtrerade yxor (de som har minst en SÄLJ-transaktion)
    sold_axe_ids = set(
        filtered_transactions.filter(type="SÄLJ").values_list("axe_id", flat=True)
    )
...
def axe_detail(request, pk):
    ...
    transactions = (
        ...
        .select_related("contact", "platform")
        .order_by("-transaction_date")
    )
    # Beräkna totala kostnader och intäkter
    total_cost = (
        transactions.filter(type="KÖP").aggregate(total=Sum("price"))["total"] or 0
    )
    total_shipping_cost = (
        transactions.filter(type="KÖP").aggregate(total=Sum("shipping_cost"))["total"]
        or 0
    )
    total_revenue = (
        transactions.filter(type="SÄLJ").aggregate(total=Sum("price"))["total"] or 0
    )
    total_shipping_revenue = (
        transactions.filter(type="SÄLJ").aggregate(total=Sum("shipping_cost"))["total"]
        or 0
    )
    ...
    if request.method == "POST" and "addTransactionForm" in request.POST.get(
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            {
                "text": axe.manufacturer.name,
                "url": f"/tillverkare/{axe.manufacturer.id}/",
            },
            {"text": f"{axe.display_id} - {axe.model}"},
        ],
        "comment_tree": build_comment_tree(axe, request.user.is_authenticated),
...
def axe_gallery(request, pk=None):
    ...
    if pk:
        current_axe = get_object_or_404(
            Axe.objects.select_related("manufacturer").prefetch_related(
                "images", "measurements"
            ),
            pk=pk,
        )
        ...
        next_axe = (
            axe_list[current_index + 1] if current_index < len(axe_list) - 1 else None
        )
        transactions = (
            Transaction.objects.filter(axe=current_axe)
            .select_related("contact", "platform")
            .order_by("-transaction_date")
        )
        total_cost = (
        ...
        total_shipping_cost = (
            transactions.filter(type="KÖP").aggregate(total=Sum("shipping_cost"))[
                "total"
            ]
            or 0
        )
        ...
        total_shipping_revenue = (
            transactions.filter(type="SÄLJ").aggregate(total=Sum("shipping_cost"))[
                "total"
            ]
            or 0
        )
...
def add_measurement(request, pk):
    ...
    if measurement_form.is_valid():
        ...
        measurement.save()
        return JsonResponse(
            {"success": True, "message": "Mått lades till framgångsrikt."}
        )
    else:
...
def add_measurements_from_template(request, pk):
    ...
    while f"measurements[{i}][name]" in request.POST:
        ...
        if name and value and unit:
            try:
                value = float(value)
                measurements_data.append({"name": name, "value": value, "unit": unit})
            except ValueError:
                return JsonResponse(
                    {"error": f"Ogiltigt värde för {name}: {value}"}, status=400
                )
        i += 1
    ...
    return JsonResponse(
        {"success": True, "message": f"{created_count} mått lades till framgångsrikt."}
    )
...
def delete_measurement(request, pk, measurement_id):
    ...
    try:
        ...
        if measurement.axe == axe:
            measurement.delete()
            return JsonResponse(
                {"success": True, "message": "Mått borttaget framgångsrikt."}
            )
        else:
            return JsonResponse(
                {"success": False, "error": "Ogiltigt mått-ID."}, status=400
            )
    except Measurement.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": "Mått hittades inte."}, status=404
        )
    ...
    try:
        ...
        if measurement.axe == axe:
            ...
            if measurement_form.is_valid():
                ...
                measurement.save()
                return JsonResponse(
                    {"success": True, "message": "Mått uppdaterat framgångsrikt."}
                )
            else:
                return JsonResponse(
                    {"success": False, "error": "Ogiltig måttdata."}, status=400
                )
        else:
            return JsonResponse(
                {"success": False, "error": "Ogiltigt mått-ID."}, status=400
...
def statistics_dashboard(request):
    ...
    top_manufacturers = Manufacturer.objects.annotate(total_axes=Count("axe")).order_by(
        "-total_axes"
    )[:5]

    # Dyraste köp (top 5)
    most_expensive_buys = (
        Transaction.objects.filter(type="KÖP")
        .select_related("axe__manufacturer")
        .order_by("-price")[:5]
    )

    # Dyraste försäljningar (top 5)
    most_expensive_sales = (
        Transaction.objects.filter(type="SÄLJ")
        .select_related("axe__manufacturer")
        .order_by("-price")[:5]
    )
    ...
    all_transactions = Transaction.objects.select_related("axe").order_by(
        "transaction_date"
    )
    ...
    latest_purchases = Transaction.objects.filter(type="KÖP").order_by(
        "-transaction_date"
    )[:5]
    latest_sales = Transaction.objects.filter(type="SÄLJ").order_by(
        "-transaction_date"
    )[:5]
...
def get_latest_axe_info(request):
    ...
    try:
        ...
        if not latest_axe:
            return JsonResponse({"success": False, "message": "Inga yxor att ta bort"})

        # Hämta kontakt från senaste transaktionen
        latest_transaction = latest_axe.transactions.order_by(
            "-transaction_date"
        ).first()
        contact_name = (
            latest_transaction.contact.name
            if latest_transaction and latest_transaction.contact
            else None
        )
        ...
        return JsonResponse(
            {"success": False, "message": f"Fel vid hämtning av information: {str(e)}"}
        )
...
def delete_latest_axe(request):
    ...
    try:
        ...
        if axe_id:
            ...
            axe = (
                Axe.objects.select_related("manufacturer")
                .prefetch_related("images", "transactions__contact")
                .order_by("-id")
                .first()
            )
            ...
            for transaction in axe.transactions.all():
                if transaction.contact:
                    ...
                    if transaction.contact.unique_axes_count == 1:
                        ...
                        messages.warning(
                            request,
                            f'Kontakt "{transaction.contact.name}" kunde inte tas bort eftersom den är kopplad till andra yxor.',
                        )
        ...
        if image_count > 0:
            if delete_images:
                ...
                if result["moved"] > 0:
                    messages.info(
                        request,
                        f'{result["moved"]} bilder flyttades till okopplade bilder (timestamp: {result["timestamp"]}).',
                    )
                if result["errors"] > 0:
                    messages.warning(
                        request, f'{result["errors"]} bilder kunde inte flyttas.'
                    )
        ...
        for contact in contacts_to_delete:
            ...
            messages.info(
                request,
                f'Kontakt "{contact.name}" togs bort eftersom den inte användes av andra yxor.',
            )
...
def unlinked_images_view(request):
    ...
    sorted_axe_groups = sorted(
        axe_groups.values(), key=lambda x: x["timestamp"], reverse=True
    )
    sorted_manufacturer_groups = sorted(
        manufacturer_groups.values(), key=lambda x: x["timestamp"], reverse=True
    )
...
def delete_unlinked_image(request):
    ...
    try:
        ...
        file_path = os.path.join(
            settings.MEDIA_ROOT, "unlinked_images", folder, filename
        )
...
def download_unlinked_images(request):
    ...
    try:
        ...
        if not os.path.exists(unlinked_base_folder):
            return JsonResponse(
                {"success": False, "error": "Inga okopplade bilder att ladda ner"}
            )
                ...
                if selected_files and selected_types:
                    ...
                    for i, filename in enumerate(selected_files):
                        if i < len(selected_types):
                            ...
                            file_path = os.path.join(
                                unlinked_base_folder, folder, filename
                            )
                            if os.path.exists(file_path) and not filename.endswith(
                                ".webp"
                            ):
                        ...
                        for file in files:
                            ...
                            if not file.endswith(".webp"):
                                ...
                                arcname = os.path.relpath(
                                    file_path, unlinked_base_folder
                                )
                                zipf.write(file_path, arcname)
        ...
        with open(tmp_file.name, "rb") as f:
            ...
            if selected_files:
                response["Content-Disposition"] = (
                    'attachment; filename="valda_bilder.zip"'
                )
            else:
                response["Content-Disposition"] = (
                    'attachment; filename="okopplade_bilder.zip"'
                )

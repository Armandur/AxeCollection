...
def search_contacts(request):
    ...
    contacts = Contact.objects.filter(
        ...
        :10
    ]  # Begränsa till 10 resultat

    results = [
        {"id": c.id, "name": c.name, "alias": c.alias or "", "email": c.email or ""}
        for c in contacts
    ]
...
def search_platforms(request):
    ...
    platforms = Platform.objects.filter(name__icontains=query)[
        :10
    ]  # Begränsa till 10 resultat
...
def _search_axes(query, request):
    ...
    try:
        id_query = int(query)
        axes = Axe.objects.filter(text_query | Q(id=id_query)).select_related(
            "manufacturer"
        )[:5]
    except ValueError:
...
def _search_transactions(query, request):
    ...
    transactions = Transaction.objects.filter(transaction_query).select_related(
...
def statistics_dashboard(request):
    ...
    top_manufacturers = Manufacturer.objects.annotate(axe_count=Count("axe")).order_by(
        "-axe_count"
    )[:5]
...
def settings_view(request):
    ...
    if request.method == "POST":
        ...
        settings.show_platforms_public = (
            request.POST.get("show_platforms_public") == "on"
        )
        ...
        settings.default_transactions_rows_public = request.POST.get(
            "default_transactions_rows_public", "15"
        )
        settings.default_manufacturers_rows_public = request.POST.get(
            "default_manufacturers_rows_public", "25"
        )

        # Visningsinställningar för inloggade användare
        settings.default_axes_rows_private = request.POST.get(
            "default_axes_rows_private", "50"
        )
        settings.default_transactions_rows_private = request.POST.get(
            "default_transactions_rows_private", "30"
        )
        settings.default_manufacturers_rows_private = request.POST.get(
        ...
        if (
            request.POST.get("external_hosts") != settings.external_hosts
            or request.POST.get("external_csrf_origins")
            != settings.external_csrf_origins
        ):
            try:
                ...
                messages.success(
                    request, "Inställningar sparade och host-konfiguration uppdaterad!"
                )
            except Exception as e:
                messages.warning(
                    request,
                    f"Inställningar sparade men host-konfiguration kunde inte uppdateras: {e}",
                )
        else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                ...
                messages.success(
                    request,
                    f'Backup-fil "{uploaded_file.name}" laddades upp framgångsrikt!',
                )
        else:
            error_message = " ".join(
...
def api_measurement_templates(request):
    ...
    try:
        templates = (
            ...
            .prefetch_related("items__measurement_type")
...
def api_measurement_types(request):
    ...
    try:
        ...
        measurement_types = MeasurementType.objects.filter(is_active=True).order_by(
            "sort_order", "name"
        )
        ...
        return JsonResponse({"success": True, "measurement_types": types_data})
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def api_create_measurement_template(request):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"success": False, "error": "Mallnamn är obligatoriskt"}, status=400
            )

        # Kontrollera om namnet redan finns
        if MeasurementTemplate.objects.filter(name=name).exists():
            return JsonResponse(
                {"success": False, "error": "En mall med detta namn finns redan"},
                status=400,
            )
        ...
        if not measurement_type_ids:
            return JsonResponse(
                {"success": False, "error": "Du måste välja minst en måtttyp"},
                status=400,
            )
        ...
        for i, mt_id in enumerate(measurement_type_ids):
            try:
                ...
                MeasurementTemplateItem.objects.create(
                    template=template, measurement_type=measurement_type, sort_order=i
                )
...
def api_create_measurement_type(request):
    ...
    try:
        ...
        if not name or not unit:
            return JsonResponse(
                {"success": False, "error": "Namn och enhet är obligatoriska"},
                status=400,
            )
        ...
        if MeasurementType.objects.filter(name=name).exists():
            return JsonResponse(
                {"success": False, "error": "En måtttyp med detta namn finns redan"},
                status=400,
            )

        # Skapa måtttypen
        measurement_type = MeasurementType.objects.create(
            name=name, unit=unit, description=description, sort_order=sort_order
        )
...
def api_update_measurement_type(request, type_id):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "error": "Endast POST-förfrågningar tillåtna"}
        )
    ...
    try:
        ...
        if MeasurementType.objects.filter(name=name).exclude(id=type_id).exists():
            return JsonResponse(
                {
                    "success": False,
                    "error": f'En måtttyp med namnet "{name}" finns redan',
                }
            )
...
def api_delete_measurement_type(request, type_id):
    ...
    try:
        ...
        template_items = MeasurementTemplateItem.objects.filter(
        ...
        return JsonResponse(
            {"success": True, "message": f'Måtttypen "{type_name}" har tagits bort'}
        )
...
def api_update_measurement_template(request, template_id):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"success": False, "error": "Mallnamn är obligatoriskt"}
            )

        # Kontrollera om namnet redan finns (exklusive nuvarande mall)
        if (
            MeasurementTemplate.objects.filter(name=name)
            .exclude(id=template_id)
            .exists()
        ):
            return JsonResponse(
                {"success": False, "error": f'En mall med namnet "{name}" finns redan'}
            )

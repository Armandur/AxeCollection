...
def contact_list(request):
    ...
    if sort == "senast":
        contacts.sort(
            key=lambda c: (
                c._latest_transaction_date is not None,
                c._latest_transaction_date,
            ),
            reverse=True,
        )
    else:
...
def contact_detail(request, pk):
    ...
    transactions = (
        ...
        .select_related("axe__manufacturer", "platform")
...
def contact_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            contact = form.save()
            messages.success(
                request, f'Kontakten "{contact.name}" har skapats framgångsrikt.'
            )
            return redirect("contact_detail", pk=contact.pk)
    else:
        form = ContactForm()

    context = {"form": form, "is_edit": False, "contact": None}
...
def contact_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            contact = form.save()
            messages.success(
                request, f'Kontakten "{contact.name}" har uppdaterats framgångsrikt.'
            )
            return redirect("contact_detail", pk=contact.pk)
    else:
        form = ContactForm(instance=contact)

    context = {"form": form, "is_edit": True, "contact": contact}
    return render(request, "axes/contact_form.html", context)
...
def contact_delete(request, pk):
    ...
    if delete_transactions:
        ...
        messages.success(
            request,
            f'Kontakten "{contact_name}" och {transactions_count} transaktioner har tagits bort.',
        )
    else:
        ...
        messages.success(
            request,
            f'Kontakten "{contact_name}" har tagits bort. {transactions_count} transaktioner har behållits men kopplingen till kontakten har tagits bort.',
        )

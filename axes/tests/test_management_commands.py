...
from axes.models import Transaction, Axe, Contact, Platform, Manufacturer, NextAxeID, ManufacturerImage
...
class DeleteManufacturerCommandTest(TestCase):
    ...
    def test_delete_manufacturer_move_axes_to_unknown(self, mock_input):
        ...
        out = StringIO()
        call_command('delete_manufacturer', self.manufacturer.id, move_axes_to_unknown=True, stdout=out)
    ...
    def test_delete_manufacturer_move_axes_to_specific(self):
        ...
        out = StringIO()
        call_command('delete_manufacturer', self.manufacturer.id, move_axes_to=target_manufacturer.id, force=True, stdout=out)
    ...
    def test_delete_manufacturer_move_axes_to_invalid_id(self):
        ...
        with self.assertRaises(Manufacturer.DoesNotExist):
            call_command('delete_manufacturer', self.manufacturer.id, move_axes_to=999, force=True, stdout=out)
    ...
    def test_delete_manufacturer_delete_axes(self):
        ...
        out = StringIO()
        call_command('delete_manufacturer', self.manufacturer.id, delete_axes=True, force=True, stdout=out)
    ...
    def test_delete_manufacturer_no_axes(self):
        ...
        out = StringIO()
        call_command('delete_manufacturer', self.manufacturer.id, force=True, stdout=out)
...
class BackupDatabaseCommandTest(TestCase):
    ...
    def test_backup_database_basic(self, mock_settings):
        ...
        mock_settings.DATABASES = {'default': {'NAME': os.path.join(temp_dir, 'test_db.sqlite3')}}
    ...
    def test_backup_database_stats_creation(self):
        ...
        self.assertEqual(db_stats['manufacturers'], 1)  # Vi skapade 1 tillverkare i setUp
        self.assertEqual(db_stats['transactions'], 2)  # Vi skapade 2 transaktioner i setUp
...
class InitMeasurementsCommandTest(TestCase):
    ...
    def test_init_measurements_creates_types(self):
        ...
        expected_types = [
            "Bladlängd", "Bladbredd", "Skaftlängd", "Skaftbredd", 
            "Total längd", "Vikt", "Bladvikt", "Skaftvikt", 
            "Handtag", "Bladtjocklek", "Öga"
        ]
    ...
    def test_init_measurements_creates_templates(self):
        ...
        for template_name in expected_templates:
            self.assertTrue(MeasurementTemplate.objects.filter(name=template_name).exists())
    ...
    def test_init_measurements_creates_template_items(self):
        ...
        standard_items = MeasurementTemplateItem.objects.filter(template=standard_template).order_by('sort_order')
        
        expected_measurements = ["Bladlängd", "Bladbredd", "Skaftlängd", "Total längd", "Vikt"]
    ...
    def test_init_measurements_creates_fallkniv_template(self):
        ...
        fallkniv_items = MeasurementTemplateItem.objects.filter(template=fallkniv_template).order_by('sort_order')
    ...
    def test_init_measurements_creates_detailed_template(self):
        ...
        detailed_items = MeasurementTemplateItem.objects.filter(template=detailed_template).order_by('sort_order')
        
        expected_measurements = [
            "Bladlängd", "Bladbredd", "Bladtjocklek", "Skaftlängd", 
            "Skaftbredd", "Total längd", "Vikt", "Bladvikt", "Skaftvikt", "Öga"
        ]
...
class UpdateHostsCommandTest(TestCase):
    ...
    def test_update_hosts_with_existing_settings(self):
        ...
        self.assertIn("CSRF_TRUSTED_ORIGINS uppdaterad: https://example.com,https://test.com", output)
        
        # Kontrollera att miljövariabler uppdaterades
        self.assertEqual(os.environ.get("ALLOWED_HOSTS"), "example.com,test.com")
        self.assertEqual(os.environ.get("CSRF_TRUSTED_ORIGINS"), "https://example.com,https://test.com")
    ...
    def test_update_hosts_combines_with_existing_env_vars(self):
        ...
        output = out.getvalue()
        self.assertIn("ALLOWED_HOSTS uppdaterad: localhost,127.0.0.1,example.com,test.com", output)
        self.assertIn("CSRF_TRUSTED_ORIGINS uppdaterad: http://localhost,http://127.0.0.1,https://example.com,https://test.com", output)
        
        # Kontrollera att miljövariabler kombinerades korrekt
        self.assertEqual(os.environ.get("ALLOWED_HOSTS"), "localhost,127.0.0.1,example.com,test.com")
        self.assertEqual(os.environ.get("CSRF_TRUSTED_ORIGINS"), "http://localhost,http://127.0.0.1,https://example.com,https://test.com")
...
class RestoreBackupCommandTest(TestCase):
    ...
    @override_settings(BASE_DIR='/tmp', MEDIA_ROOT='/tmp/media', DATABASES={'default': {'NAME': 'test_db.sqlite3'}})
    def test_restore_backup_file_not_found(self):
        """Test att kommandot hanterar saknade backup-filer"""
        out = StringIO()
        call_command('restore_backup', 'nonexistent_file.sqlite3', confirm=True, stdout=out)
        output = out.getvalue()
    ...
    def test_restore_from_zip_with_media(self, mock_copytree, mock_copy2):
        ...
        out = StringIO()
        call_command('restore_backup', self.zip_backup_file, confirm=True, include_media=True, stdout=out)
        output = out.getvalue()
    ...
    def test_fix_image_paths(self, mock_connection):
        ...
        mock_cursor.fetchall.return_value = [(1, "test\\image.jpg"), (2, "normal/image.jpg")]
...
class GenerateTestDataCommandTest(TestCase):
    ...
    def test_generate_test_data_with_clear(self):
        ...
        self.assertFalse(Manufacturer.objects.filter(name="Existing Manufacturer").exists())
    ...
    def test_generate_test_data_custom_counts(self):
        ...
        out = StringIO()
        call_command('generate_test_data', axes=5, manufacturers=3, contacts=4, stdout=out)
    ...
    def test_generate_test_data_creates_measurement_types(self):
        ...
        for expected_type in expected_types:
            self.assertTrue(MeasurementType.objects.filter(name__icontains=expected_type).exists())
    ...
    def test_generate_test_data_creates_measurement_templates(self):
        ...
        for expected_template in expected_templates:
            self.assertTrue(MeasurementTemplate.objects.filter(name__icontains=expected_template).exists())
    ...
    def test_generate_test_data_success_message_format(self):
        ...
        out = StringIO()
        call_command('generate_test_data', axes=10, manufacturers=5, contacts=8, stdout=out)

...
class ViewsManufacturerTestCase(TestCase):
    def setUp(self):
        ...
        self.parent_manufacturer = Manufacturer.objects.create(
            name="Parent Manufacturer", manufacturer_type="TILLVERKARE"
        )
        ...
        self.sub_manufacturer = Manufacturer.objects.create(
            name="Sub Manufacturer", parent=self.manufacturer, manufacturer_type="SMED"
        )
...
class ManufacturerListViewTest(ViewsManufacturerTestCase):
    ...
    def test_manufacturer_list_view_statistics(self):
        """Testa att tillverkarlistan visar statistik"""
        response = self.client.get("/tillverkare/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_manufacturer_list_view_with_country_codes(self):
        """Testa att tillverkarlistan visar landskoder korrekt"""
        ...
        self.assertEqual(swedish_manufacturer.country_code, "SE")
        self.assertEqual(finnish_manufacturer.country_code, "FI")
        self.assertIsNone(no_code_manufacturer.country_code)
...
class ManufacturerEditViewTest(ViewsManufacturerTestCase):
    ...
    def test_manufacturer_edit_view_valid_post(self):
        ...
        response = self.client.post(
            f"/tillverkare/{self.manufacturer.id}/redigera/", data
        )
...
class ManufacturerDeleteViewTest(ViewsManufacturerTestCase):
    ...
    def test_manufacturer_delete_view_login_access(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        data = {"axe_action": "move", "delete_images": "false"}
        response = self.client.post(
            f"/tillverkare/{self.manufacturer.id}/ta-bort/", data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_manufacturer_delete_view_with_axes(self):
        ...
        unknown_manufacturer = Manufacturer.objects.filter(
    ...
    def test_delete_manufacturer_with_sub_manufacturers(self):
...
class ManufacturerAJAXViewsTest(ViewsManufacturerTestCase):
    def test_edit_manufacturer_information(self):
        ...
        data = {"information": "Updated information via AJAX"}
        response = self.client.post(
    ...
    def test_edit_manufacturer_name(self):
        ...
        data = {"name": "Updated Name via AJAX"}
        response = self.client.post(
    ...
    def test_check_manufacturer_name(self):
        ...
        response = self.client.get(
            "/tillverkare/check-name/", {"name": "Test Manufacturer"}
        )
        self.assertEqual(response.status_code, 200)
        ...
        # Testa nytt namn
        response = self.client.get(
            "/tillverkare/check-name/", {"name": "New Manufacturer"}
        )
        self.assertEqual(response.status_code, 200)
...
class MoveManufacturerImagesTest(ViewsManufacturerTestCase):
    def test_move_manufacturer_images_to_unlinked(self):
        ...
        second_image.image = SimpleUploadedFile(
            "test_image2.jpg", b"fake-image-content-2", content_type="image/jpeg"
        )
        second_image.save()

        # Kontrollera att bilderna finns innan flytt
        self.assertTrue(
            ManufacturerImage.objects.filter(manufacturer=self.manufacturer).exists()
        )

        # Flytta bilderna
        moved_count, error_count = move_manufacturer_images_to_unlinked(
            self.manufacturer
        )
    ...
    def test_move_manufacturer_images_to_unlinked_with_special_characters(self):
        ...
        special_manufacturer = Manufacturer.objects.create(
            name="Test & Manufacturer (Special) - 123", manufacturer_type="TILLVERKARE"
        )
        ...
        special_image.image = SimpleUploadedFile(
            "special_image.jpg", b"fake-image-content", content_type="image/jpeg"
        )
        special_image.save()
        ...
        self.assertFalse(
            ManufacturerImage.objects.filter(manufacturer=special_manufacturer).exists()
        )
...
class ManufacturerImageAJAXViewsTest(ViewsManufacturerTestCase):
    ...
    def test_reorder_manufacturer_images_valid_data(self):
        ...
        data = {"manufacturer_id": self.manufacturer.id, "order_data": order_data}
...
class ManufacturerLinkAJAXViewsTest(ViewsManufacturerTestCase):
    ...
    def test_reorder_manufacturer_links_nonexistent_link(self):
        ...
        order_data = [{"link_id": 99999, "order": 1}]  # Icke-existerande länk

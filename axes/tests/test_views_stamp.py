...
class StampViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.stamp_symbol = StampSymbol.objects.create(
            name="Test Symbol",
            description="Test symbol beskrivning"
        )
    ...
    def test_stamp_image_delete_requires_login(self):
        ...
        response = self.client.get(reverse('stamp_image_delete', args=[self.stamp.id, self.stamp_image.id]))
        self.assertEqual(response.status_code, 302)

    def test_stamp_image_edit_requires_login(self):
        """Testa att stamp_image_edit kräver inloggning"""
        response = self.client.get(reverse('stamp_image_edit', args=[self.stamp.id, self.stamp_image.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_remove_axe_stamp_requires_login(self):
        ...
        response = self.client.get(reverse('remove_axe_stamp', args=[self.axe.id, self.axe_stamp.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_mark_axe_image_as_stamp_requires_login(self):
        ...
        response = self.client.get(reverse('mark_axe_image_as_stamp', args=[self.axe.id, 1]))
        self.assertEqual(response.status_code, 302)

    def test_unmark_axe_image_stamp_requires_login(self):
        """Testa att unmark_axe_image_stamp kräver inloggning"""
        response = self.client.get(reverse('unmark_axe_image_stamp', args=[self.axe.id, 1]))
        self.assertEqual(response.status_code, 302)

    def test_edit_axe_image_stamp_requires_login(self):
        """Testa att edit_axe_image_stamp kräver inloggning"""
        response = self.client.get(reverse('edit_axe_image_stamp', args=[self.axe.id, 1]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_set_primary_stamp_image_requires_login(self):
        ...
        response = self.client.get(reverse('set_primary_stamp_image', args=[self.stamp.id, self.stamp_image.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_update_axe_image_stamp_show_full_requires_login(self):
        ...
        response = self.client.get(reverse('update_axe_image_stamp_show_full', args=[1]))
        self.assertEqual(response.status_code, 302)

    def test_edit_axe_stamp_requires_login(self):
        """Testa att edit_axe_stamp kräver inloggning"""
        response = self.client.get(reverse('edit_axe_stamp', args=[self.axe.id, self.axe_stamp.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_edit_axe_image_stamp_via_axe_stamp_requires_login(self):
        ...
        response = self.client.get(reverse('edit_axe_image_stamp_via_axe_stamp', args=[self.axe.id, 1]))
        self.assertEqual(response.status_code, 302)

    def test_remove_axe_image_stamp_requires_login(self):
        """Testa att remove_axe_image_stamp kräver inloggning"""
        response = self.client.get(reverse('remove_axe_image_stamp', args=[self.axe.id, 1]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_transcription_edit_requires_login(self):
        ...
        response = self.client.get(reverse('transcription_edit', args=[self.stamp.id, self.transcription.id]))
        self.assertEqual(response.status_code, 302)

    def test_transcription_delete_requires_login(self):
        """Testa att transcription_delete kräver inloggning"""
        response = self.client.get(reverse('transcription_delete', args=[self.stamp.id, self.transcription.id]))
        self.assertEqual(response.status_code, 302)

    def test_stamp_transcriptions_view(self):
        """Testa stamp_transcriptions view"""
        response = self.client.get(reverse('stamp_transcriptions', args=[self.stamp.id]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "TEST STÄMPEL")
    ...
    def test_stamp_symbol_update_requires_login(self):
        ...
        response = self.client.get(reverse('stamp_symbol_update', args=[self.stamp_symbol.id]))
        self.assertEqual(response.status_code, 302)

    def test_stamp_symbol_delete_requires_login(self):
        """Testa att stamp_symbol_delete kräver inloggning"""
        response = self.client.get(reverse('stamp_symbol_delete', args=[self.stamp_symbol.id]))
        self.assertEqual(response.status_code, 302)

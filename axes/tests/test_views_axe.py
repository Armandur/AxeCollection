...
from axes.models import Axe, Contact, Manufacturer, Platform, Transaction, Settings, AxeImage, Measurement, MeasurementTemplate, MeasurementType, NextAxeID
...
class AJAXEndpointsTest(ViewsAxeTestCase):
    def test_update_axe_status_requires_login(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/status/', {'status': 'MOTTAGEN'})
        self.assertEqual(response.status_code, 302)  # Redirect till login
    ...
    def test_update_axe_status_invalid_status(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/status/', {'status': 'INVALID'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_measurement_requires_login(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/matt/', {
            'name': 'Length',
            'value': '10.5',
            'unit': 'cm'
        })
    ...
    def test_add_measurement_with_login(self):
        ...
        if response.status_code == 200:
            ...
            measurement = Measurement.objects.filter(axe=self.axe, name='Length').first()
    ...
    def test_add_measurements_from_template_requires_login(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/matt/mall/', {
    ...
    def test_update_measurement_requires_login(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/matt/{measurement.id}/update/', {
    ...
    def test_update_measurement_with_login(self):
        ...
        response = self.client.post(f'/yxor/{self.axe.id}/matt/{measurement.id}/update/', data)
        # Kontrollera att antingen 200 (framgång) eller 400 (valideringsfel)
        self.assertIn(response.status_code, [200, 400])
...
class UnlinkedImagesAJAXTest(ViewsAxeTestCase):
    def test_delete_unlinked_image_requires_login(self):
        ...
        response = self.client.post('/okopplade-bilder/ta-bort/', {
            'filename': 'test.jpg',
            'type': 'axe'
        })
...
class MoveImagesToUnlinkedFolderTest(ViewsAxeTestCase):
    ...
    def test_move_images_to_unlinked_folder_delete_images(self):
        ...
        result = move_images_to_unlinked_folder(AxeImage.objects.filter(id=image.id), delete_images=True)
        self.assertEqual(result['moved'], 0)

...
class StampSymbolIntegrationTest(TestCase):
    ...
    def test_symbol_usage_with_stamps(self):
        ...
        self.assertIn("Kunglig Krona", symbol_names)
        self.assertIn("Femuddig Stjärna", symbol_names)

    def test_symbol_type_consistency(self):
        """Testa konsistens i symboltyper"""
        # Definiera alla giltiga symboltyper
        valid_types = [
            "crown", "cannon", "star", "cross", "shield", "anchor",
            "flower", "leaf", "arrow", "circle", "square", "triangle",
            "diamond", "heart", "other"
        ]
    ...
    def test_predefined_symbols_management(self):
        ...
        delete_url = reverse("stamp_symbol_delete", kwargs={"symbol_id": user_symbol.id})
        response = self.client.post(delete_url)
        self.assertEqual(response.status_code, 302)

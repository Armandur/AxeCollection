    ...
    def handle(self, *args, **options):
        ...
        for axe_stamp in axe_stamps:
            ...
            if current_value in swedish_to_english:
                ...
                self.stdout.write(
                    f"Fixing yxa {axe_stamp.axe.display_id} - stämpel {axe_stamp.stamp.name}:"
                )
        ...
        for axe_stamp in AxeStamp.objects.all():
            if axe_stamp.uncertainty_level not in valid_choices:
                self.stdout.write(
                    f"VARNING: Yxa {axe_stamp.axe.display_id} har fortfarande ogiltigt värde: '{axe_stamp.uncertainty_level}'"
                )
            else:
                self.stdout.write(
                    f"✓ Yxa {axe_stamp.axe.display_id}: '{axe_stamp.uncertainty_level}' -> '{axe_stamp.get_uncertainty_level_display()}'"
                )

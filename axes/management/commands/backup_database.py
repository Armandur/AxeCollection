...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['compress']:
            self.create_compressed_backup(backup_dir, timestamp, db_backup_path, media_backup_path, stats)
        else:
    ...
    def create_backup_stats(self):
        ...
        from axes.models import Axe, Contact, Manufacturer, Transaction, Measurement, AxeImage, ManufacturerImage, ManufacturerLink
        ...
        total_buy_value = sum(t.price for t in Transaction.objects.filter(type='KÖP') if t.price)
        total_sell_value = sum(t.price for t in Transaction.objects.filter(type='SÄLJ') if t.price)
        ...
        stats['financial'] = {
            ...
            'net_value': float(total_sell_value - total_buy_value) if total_sell_value and total_buy_value else 0
        }
    ...
    def create_compressed_backup(self, backup_dir, timestamp, db_backup_path, media_backup_path, stats):

...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('csv_dir', type=str, help='Sökväg till mappen med CSV-filer')
        parser.add_argument('--reset', action='store_true', help='Rensa all data (Yxor, Kontakter, Transaktioner, etc.) innan import')
        ...
        for row in reader:
            if len(row) >= 5:
                ...
                if len(row) > 5:
                    ...
                    if image_col:
                        image_filenames = [img.strip() for img in image_col.split(';') if img.strip()]
                ...
                if manufacturer_id and manufacturer_id in self.manufacturer_map:
                    ...
                    from axes.models import AxeImage  # Importera här för att undvika cirkulärt beroende
                    ...
                    for img_filename in image_filenames:
                        ...
                        if os.path.exists(src_path) and not os.path.exists(dest_path):
                            ...
                            self.stdout.write(f'  Kopierade bild: {src_path} -> {dest_path}')
                    ...
                    self.stdout.write(f'Kunde inte hitta tillverkare med ID {manufacturer_id} för yxa {model}')
    ...
    def import_transactions(self, csv_dir):
        ...
        for row in reader:
            if len(row) >= 8:
                ...
                transaction_type = row[9].strip('"') if len(row) > 9 and row[9] else 'KÖP'
                ...
                try:
                    ...
                    self.stdout.write(f'Fel vid import av transaktion {transaction_id}: {e}')
            ...
            if len(row) >= 5:
                ...
                try:
                    ...
                    if axe_id and axe_id in self.axe_map:
                        ...
                        self.stdout.write(f'Kunde inte hitta yxa med ID {axe_id} för mätning')
                        continue
                    
                    # Konvertera värde
                    value = Decimal(value_str.replace(',', '.')) if value_str else Decimal('0')
                    ...
                    self.stdout.write(f'Fel vid import av mätning {measurement_id}: {e}')
    ...
    def import_manufacturerimages(self, csv_dir):
        ...
        for row in reader:
            if len(row) >= 4:
                ...
                description = row[5].strip('"') if len(row) > 5 and row[5] else ''
                if manufacturer_id and manufacturer_id in self.manufacturer_map and img_filename:
                    manufacturer = self.manufacturer_map[manufacturer_id]
                    src_path = os.path.join(csv_dir, 'manufacturer_images', img_filename)
                    dest_path = os.path.join(media_manuf_dir, img_filename)
                    image_field_path = os.path.join('manufacturer_images', img_filename)
                    if os.path.exists(src_path) and not os.path.exists(dest_path):
                        shutil.copy2(src_path, dest_path)
                        self.stdout.write(f'  Kopierade tillverkarbild: {src_path} -> {dest_path}')
                    elif not os.path.exists(src_path):

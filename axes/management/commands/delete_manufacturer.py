...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if axe_count > 0 and not options['move_axes_to_unknown'] and not options['delete_axes'] and not options['move_axes_to']:
            self.stdout.write(
                self.style.ERROR(
        ...
        if not options['force']:
            if axe_count > 0:
                if options['delete_axes']:
                    confirm = input(f'Är du säker på att du vill ta bort tillverkaren och alla {axe_count} yxor? (ja/nej): ')
                elif options['move_axes_to']:
                    try:
                        target_manufacturer = Manufacturer.objects.get(id=options['move_axes_to'])
                        confirm = input(f'Är du säker på att du vill flytta {axe_count} yxor till "{target_manufacturer.name}"? (ja/nej): ')
                    except Manufacturer.DoesNotExist:
                        self.stdout.write(self.style.ERROR(f'Tillverkare med ID {options["move_axes_to"]} finns inte'))
                        return
                else:
                    confirm = input(f'Är du säker på att du vill flytta {axe_count} yxor till "Okänd tillverkare"? (ja/nej): ')
            else:
                confirm = input(f'Är du säker på att du vill ta bort tillverkaren? (ja/nej): ')
        ...
        if axe_count > 0:
            if options['delete_axes']:
                ...
                target_manufacturer = Manufacturer.objects.get(id=options['move_axes_to'])
                self.stdout.write(f'Flyttar {axe_count} yxor till "{target_manufacturer.name}"...')
                ...
                self.stdout.write(f'Flyttar {axe_count} yxor till "{unknown_manufacturer.name}"...')
    ...
    def move_manufacturer_images_to_unlinked(self, manufacturer):
        ...
        for index, manufacturer_image in enumerate(sorted_images):
            try:
                ...
                error_count += 1
                self.stdout.write(f'Fel vid flytt av bild {manufacturer_image.image.name}: {e}')
        
        if moved_count > 0:
            self.stdout.write(
                self.style.SUCCESS(f'Flyttade {moved_count} tillverkarbilder till okopplade bilder')
            )

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Återställning slutförd!"))
    ...
    def restore_database(self, db_backup_path):
        ...
        if os.path.exists(db_name):
            ...
            self.stdout.write(
                f"  Skapade backup av nuvarande databas: {current_backup}"
            )

        # Kopiera backup-databasen
        shutil.copy2(db_backup_path, db_name)
        self.stdout.write(
            f"  Återställde databas från: {os.path.basename(db_backup_path)}"
        )
    ...
    def restore_media_files(self, media_backup_dir):
        ...
        if os.path.exists(settings.MEDIA_ROOT):
            ...
            if os.path.exists(current_media_backup):
                ...
                self.stdout.write(
                    f"  Tog bort befintlig backup: {current_media_backup}"
                )

            shutil.copytree(settings.MEDIA_ROOT, current_media_backup)
            self.stdout.write(
                f"  Skapade backup av nuvarande media: {current_media_backup}"
            )
    ...
    def fix_image_paths(self):
        ...
        for row_id, image_path in rows:
            if "\\" in image_path:
                ...
                cursor.execute(
                    "UPDATE axes_axeimage SET image = %s WHERE id = %s",
                    [new_path, row_id],
                )
                backslash_fixes += 1
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM axes_axeimage WHERE image LIKE '/app/media/%'"
        )
        ...
        if count_with_app_media > 0:
            cursor.execute(
                "UPDATE axes_axeimage SET image = REPLACE(image, '/app/media/', '')"
            )
            self.stdout.write(
                f"  Tog bort /app/media/ prefix från {count_with_app_media} sökvägar"
            )
        ...
        if getattr(settings, "DEBUG", True):
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM axes_axeimage WHERE image LIKE 'media/%'"
            )
            count_with_media_prefix = cursor.fetchone()[0]

            if count_with_media_prefix > 0:
                cursor.execute(
                    "UPDATE axes_axeimage SET image = REPLACE(image, 'media/', '')"
                )
                self.stdout.write(
                    f"  Tog bort media/ prefix från {count_with_media_prefix} sökvägar (utvecklingsmiljö)"
                )
            ...
            if count_with_media_prefix > 0:
                ...
                self.stdout.write(
                    f"  Tog bort media/ prefix från {count_with_media_prefix} sökvägar (produktionsmiljö)"
                )
                fixes_made += count_with_media_prefix

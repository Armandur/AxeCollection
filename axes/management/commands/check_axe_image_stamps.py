    ...
    def handle(self, *args, **options):
        ...
        if count > 0:
            ...
            for ais in AxeImageStamp.objects.all()[:5]:
                ...
                self.stdout.write(f'    Coordinates: x={ais.x_coordinate}, y={ais.y_coordinate}, w={ais.width}, h={ais.height}')
        ...
        if axe_stamp_count > 0:
            ...
            for as_obj in AxeStamp.objects.all()[:3]:
                ...
                image_stamps = AxeImageStamp.objects.filter(stamp=as_obj.stamp, axe_image__axe=as_obj.axe)
                self.stdout.write(f'    Associated AxeImageStamp records: {image_stamps.count()}')
                for ais in image_stamps:
                    self.stdout.write(f'      - {ais.axe_image.axe.display_id} image {ais.axe_image.id}: x={ais.x_coordinate}, y={ais.y_coordinate}') 
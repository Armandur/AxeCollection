...
class Command(BaseCommand):
    help = 'Kontrollera AxeStamp-värden för att identifiera problem med uncertainty_level'
    ...
    def handle(self, *args, **options):
        ...
        for axe_stamp in axe_stamps:
            ...
            self.stdout.write(f"  uncertainty_level (raw): '{axe_stamp.uncertainty_level}'")
            self.stdout.write(f"  get_uncertainty_level_display(): '{axe_stamp.get_uncertainty_level_display()}'")
            ...
            if axe_stamp.uncertainty_level not in valid_choices:
                self.stdout.write(f"  VARNING: Ogiltigt värde! Giltiga värden: {valid_choices}")
            else:
        ...
        for axe_id in [52, 53]:
            try:
                ...
                for axe_stamp in axe_stamps:
                    ...
                    self.stdout.write(f"  uncertainty_level: '{axe_stamp.uncertainty_level}'")
                    self.stdout.write(f"  display: '{axe_stamp.get_uncertainty_level_display()}'")

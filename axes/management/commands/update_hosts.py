    ...
    def handle(self, *args, **options):
        try:
            ...
            if settings_obj.external_csrf_origins:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'CSRF_TRUSTED_ORIGINS uppdaterad: {combined_origins}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING('Inga externa CSRF-origins konfigurerade i databasen')
                )
                
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Fel vid uppdatering av hosts: {e}')
            ) 
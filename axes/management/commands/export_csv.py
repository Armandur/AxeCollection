...
from axes.models import Axe, Manufacturer, Contact, Platform, Transaction, Measurement, AxeImage, ManufacturerLink
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('Export klar! Filer finns i exported_csv/'))

    def export_manufacturers(self):
        with open(os.path.join(EXPORT_DIR, 'Manufacturer.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    def export_contacts(self):
        with open(os.path.join(EXPORT_DIR, 'Kontakt.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'name', 'email', 'phone', 'alias', 'street', 'postal_code', 'city', 'country', 'comment', 'is_naj_member'])
    ...
    def export_platforms(self):
        with open(os.path.join(EXPORT_DIR, 'Platform.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    def export_axes(self):
        with open(os.path.join(EXPORT_DIR, 'Axe.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'manufacturer_id', 'manufacturer_name', 'model', 'comment'])
    ...
    def export_transactions(self):
        with open(os.path.join(EXPORT_DIR, 'Transaction.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'axe_id', 'axe_model', 'contact_id', 'contact_name', 'platform_id', 'platform_name', 'transaction_date', 'type', 'price', 'shipping_cost', 'comment'])
            for t in Transaction.objects.select_related('axe', 'contact', 'platform').all():
                writer.writerow([
                    t.id,
    ...
    def export_measurements(self):
        with open(os.path.join(EXPORT_DIR, 'Measurement.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'axe_id', 'axe_model', 'name', 'value', 'unit'])
            for m in Measurement.objects.select_related('axe').all():
                writer.writerow([
                    m.id, m.axe.id if m.axe else '', m.axe.model if m.axe else '',
                    m.name, m.value, m.unit
                ])

    def export_axeimages(self):
        with open(os.path.join(EXPORT_DIR, 'AxeImage.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    def export_manufacturerlinks(self):
        with open(os.path.join(EXPORT_DIR, 'ManufacturerLink.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'manufacturer_id', 'manufacturer_name', 'title', 'url', 'link_type', 'description', 'is_active', 'created_at', 'updated_at'])
    ...
    def export_manufacturerimages(self):
        ...
        with open(os.path.join(EXPORT_DIR, 'ManufacturerImage.csv'), 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'manufacturer_id', 'manufacturer_name', 'image', 'caption', 'description'])

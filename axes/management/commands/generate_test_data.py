...
from axes.models import (
    Manufacturer, Axe, AxeImage, ManufacturerImage, ManufacturerLink,
    Contact, Platform, Transaction, Measurement, MeasurementType,
    MeasurementTemplate, MeasurementTemplateItem, Settings, NextAxeID
)
...
class Command(BaseCommand):
        ...
        for name, description in templates:
            ...
            if created:
                ...
                if name == 'Standardyxa':
                    ...
                    measurement_types = MeasurementType.objects.filter(
                        name__in=['Vikt', 'Skaftlängd', 'Eggbredd', 'Huvudvikt', 'Ögats bredd', 'Ögats höjd']
                    )
    ...
    def create_contacts(self, count):
        """Skapa kontakter"""
        countries = [
            ('Sverige', 'SE'), ('Finland', 'FI'), ('Norge', 'NO'), ('Danmark', 'DK'),
            ('Tyskland', 'DE'), ('USA', 'US'), ('Kanada', 'CA'), ('Storbritannien', 'GB'),
            ('Polen', 'PL'), ('Tjeckien', 'CZ'), ('Österrike', 'AT'), ('Schweiz', 'CH')
        ]
        
        first_names = ['Erik', 'Anna', 'Lars', 'Maria', 'Johan', 'Karin', 'Anders', 'Eva', 'Mikael', 'Sofia']
        last_names = ['Andersson', 'Johansson', 'Karlsson', 'Nilsson', 'Eriksson', 'Larsson', 'Olsson', 'Persson', 'Svensson', 'Gustafsson']
        ...
        for i in range(count):
            ...
            contact = Contact.objects.create(
                ...
                city=random.choice(['Stockholm', 'Göteborg', 'Malmö', 'Uppsala', 'Västerås', 'Örebro', 'Linköping', 'Helsingborg']),
                country=country,
    ...
    def create_axes(self, manufacturers, count):
        """Skapa yxor"""
        axe_models = [
            'Slöjdyxa', 'Huggyxa', 'Fällyxa', 'Klyvyxa', 'Timmerbila',
            'Handyxa', 'Tapphålsyxa', 'Tväryxa', 'Tjäckla', 'Snickaryxa'
        ]
        
        comments = [
            'Fin kvalitetsyxa', 'Handgjord', 'Vintage', 'Ny i förpackning',
            'Lite använd', 'Bra skick', 'Kollektion', 'Sällsynt modell',
            'Begagnad men bra', 'Perfekt för slöjd', 'Professionell kvalitet',
            'Gammal svensk yxa', 'Traditionell tillverkning', 'Kollektionsföremål'
        ]
        ...
        for axe in axes:
            ...
            for i, transaction_date in enumerate(transaction_dates):
                ...
                Transaction.objects.create(
                    ...
                    platform=random.choice(platforms) if random.random() > 0.1 else None,
                    transaction_date=transaction_date,
                    type=transaction_type,
                    price=price,
                    shipping_cost=shipping_cost,
                    comment=random.choice([
                        'Snabb leverans', 'Bra förpackning', 'Som beskrivet',
                        'Kollektion', 'Auktion', 'Direktköp', 'Förhandling'
    ...
    def create_demo_image(self, width=800, height=600, text="Demo Bild", bg_color=(240, 240, 240), text_color=(100, 100, 100)):
            ...
            if random.random() > 0.6:
                ...
                for i in range(num_images):
                    ...
                    img_io = self.create_demo_image(
                        ...
                        text_color=(random.randint(50, 150), random.randint(50, 150), random.randint(50, 150))
                    )
                    
                    # Skapa filnamn enligt namnstandard: Tillverkarnamn_Stämpel.jpg
                    image_letter = self.get_image_letter(i)
                    safe_name = manufacturer.name.replace(' ', '_').replace(',', '').replace('.', '')
                    ...
                    # Spara bilden
                    manufacturer_image.image.save(filename, ContentFile(img_io.getvalue()), save=True)
            ...
            if random.random() > 0.4:
                ...
                for i in range(num_links):
                    ...
                    ManufacturerLink.objects.create(
                        ...
                        link_type=link_type,
                        description=description,
                        is_active=random.choice([True, True, True, False]),  # 75% chans att vara aktiv
                        order=i
                    )
            ...
            if random.random() > 0.3:
                ...
                for i in range(num_images):
                    ...
                    img_io = self.create_demo_image(
                        ...
                        bg_color=(random.randint(180, 220), random.randint(180, 220), random.randint(180, 220)),
                    ...
                    # Spara bilden
                    axe_image.image.save(filename, ContentFile(img_io.getvalue()), save=True)

...
class Command(BaseCommand):
    ...
    def create_specific_axes_with_real_images(self, manufacturers, no_stamp_images=False):
        ...
        for axe_data in specific_axes:
            ...
            self.stdout.write(f"  Skapade yxa {axe.display_id} ({axe.model}) från {axe.manufacturer.name}")
            ...
            for i, image_filename in enumerate(axe_data["images"]):
                ...
                if os.path.exists(source_path):
                    ...
                    with open(source_path, 'rb') as f:
                        axe_image.image.save(image_filename, ContentFile(f.read()), save=True)
                    
                    self.stdout.write(f"    Kopierade {image_filename} till yxa {axe.display_id} (AxeImage ID: {axe_image.id})")
                else:
                    self.stdout.write(f"    VARNING: Bildfil {image_filename} hittades inte!")
            
            self.stdout.write(f"    Skapade {len(axe_images)} bilder för yxa {axe.display_id}")
        ...
        for axe in created_axes:
            self.stdout.write(f"  - Yxa {axe.display_id}: {axe.model} från {axe.manufacturer.name}")
            axe_images = AxeImage.objects.filter(axe=axe).order_by('order')
            for axe_image in axe_images:
                self.stdout.write(f"    * Bild {axe_image.order+1}: {axe_image.image.name} (ID: {axe_image.id})")
    ...
    def create_specific_stamps(self, manufacturers):
        ...
        for stamp_data in stamps_data:
            if stamp_data["manufacturer"]:
                ...
                self.stdout.write(f"  Skapade stämpel: {stamp.name} för {stamp.manufacturer.name}")
            else:
                self.stdout.write(f"  VARNING: Tillverkare för stämpel {stamp_data['name']} hittades inte!")
    ...
    def create_axe_stamp_connections(self):
        ...
        if not specific_axes:
            ...
            if len(first_axes) >= 4:
                ...
                for display_id, axe in specific_axes.items():
                    self.stdout.write(f"  Mappade yxa {axe.display_id} (ID {axe.id}) till display_id {display_id}")
        
        # Definiera realistiska kopplingar baserat på faktiska data
        realistic_connections = [
            # Yxa 33 (Hjärtumyxa från Johan Jonsson)
            {"axe_display_id": "33", "stamp_name": "J.JONSSON", "position": "Vid nacken", "uncertainty_level": "certain"},
            
            # Yxa 166 (Klyvyxa från Johan Jonsson) 
            {"axe_display_id": "166", "stamp_name": "J.JONSSON", "position": "Vid nacken", "uncertainty_level": "certain"},
            
            # Yxa 152 (Täljbila från Hults bruk)
            {"axe_display_id": "152", "stamp_name": "HM", "position": "Vid nacken", "uncertainty_level": "certain"},
            
            # Yxa 20 (Hjärtumyxa från Johan Skog)
            {"axe_display_id": "20", "stamp_name": "J.SKOG HJÄRTUM", "position": "Mitt på kinden", "uncertainty_level": "certain"},
            {"axe_display_id": "20", "stamp_name": "S", "position": "På bladet", "uncertainty_level": "certain"},
        ]
        ...
        for connection_data in realistic_connections:
            try:
                ...
                if not axe:
                    self.stdout.write(f"  VARNING: Yxa {connection_data['axe_display_id']} hittades inte!")
                    continue
                ...
                self.stdout.write(f"  Kopplade {axe_stamp.stamp.name} till yxa {axe_stamp.axe.display_id} ({connection_data['position']})")
                
            except Stamp.DoesNotExist as e:
                self.stdout.write(f"  VARNING: Kunde inte koppla {connection_data['stamp_name']} till yxa {connection_data['axe_display_id']}: {e}")
            ...
            if matching_stamps.exists():
                ...
                for stamp in selected_stamps:
                    ...
                    axe_stamp = AxeStamp.objects.create(
                        ...
                        uncertainty_level=random.choices(uncertainty_levels, weights=weights)[0],
                        comment=f"Stämpel från {stamp.manufacturer.name} på {axe.model}"
                    )
                    created_connections.append(axe_stamp)
                    self.stdout.write(f"  Kopplade {axe_stamp.stamp.name} till yxa {axe_stamp.axe.display_id}")
    ...
    def convert_existing_axe_stamps_to_stamp_images(self):
        ...
        self.stdout.write("Konverterar befintliga AxeStamp-markeringar till StampImage med koordinater...")
        
        # Rensa gamla StampImage-objekt som skapats av tidigare körningar
        old_stamp_images = StampImage.objects.filter(
            image_type="axe_mark",
            comment__startswith="Genererad från AxeStamp"
        )
        if old_stamp_images.exists():
            deleted_count = old_stamp_images.count()
            old_stamp_images.delete()
            self.stdout.write(f"Rensade {deleted_count} gamla StampImage-objekt från tidigare körningar")
        ...
        for axe_stamp in axe_stamps:
            ...
            if not axe_images.exists():
                self.stdout.write(f"  Ingen bild hittad för yxa {axe_stamp.axe.display_id}")
                continue
            ...
            if existing_stamp_images.exists():
                self.stdout.write(f"  StampImage redan finns för {axe_stamp.stamp.name} på yxa {axe_stamp.axe.display_id}, hoppar över")
                continue
            ...
            for axe_image in axe_images:
                ...
                # Spara demo-bilden
                stamp_image.image.save(f"stamp_demo_{stamp_image.id}.jpg", demo_image, save=True)
                
                self.stdout.write(f"  Skapade StampImage för {axe_stamp.stamp.name} på yxa {axe_stamp.axe.display_id} vid ({x_coord}%, {y_coord}%)")
                created_count += 1
        
        self.stdout.write(f"Skapade {created_count} StampImage-objekt från befintliga markeringar")
    ...
    def _get_manual_coordinates(self, axe_display_id, stamp_name):
        ...
        manual_coordinates = {
            ...
            51: {
                "J.JONSSON": {"x": 33.0, "y": 40.0, "width": 27.0, "height": 14.0}
            },
            # Yxa 52 (display_id 52) - Klyvyxa från Johan Jonsson  
            52: {
                "J.JONSSON": {"x": 40.0, "y": 17.0, "width": 9.0, "height": 3.0},  # Bild 1
                "J.JONSSON_2": {"x": 37.0, "y": 19.0, "width": 18.0, "height": 7.0}   # Bild 2
            },
                                          # Yxa 53 (display_id 53) - Täljbila från Hults bruk
               53: {
                   "HM": {"x": 43.0, "y": 50.0, "width": 18.0, "height": 12.0}
               },
        ...
        if axe_display_id in manual_coordinates and stamp_name in manual_coordinates[axe_display_id]:
    ...
    def _create_stamp_images_with_manual_coordinates(self, axes):
        ...
        stamp_assignments = {
            ...
            52: [("J.JONSSON", 0), ("J.JONSSON_2", 1)],  # Yxa 52: J.JONSSON på bild 1, J.JONSSON_2 på bild 2
                ...
                for stamp_name, image_index in stamp_assignments[axe_display_id]:
                    if image_index < len(axe_images):
                        ...
                        try:
                            ...
                            self.stdout.write(f"    VARNING: Stämpel '{actual_stamp_name}' finns inte!")
                            continue
                        ...
                        x_coord, y_coord, width, height = self._get_manual_coordinates(axe_display_id, stamp_name)
                        ...
                        stamp_image.image.save(f"stamp_manual_{stamp_image.id}.jpg", demo_image, save=True)
                        
                        self.stdout.write(f"    Skapade StampImage för {stamp_name} på bild {image_index+1} (x={x_coord}%, y={y_coord}%, w={width}%, h={height}%)")
                        created_count += 1
                    else:
                        self.stdout.write(f"    VARNING: Bildindex {image_index} finns inte för yxa {axe.display_id}")
        
        self.stdout.write(f"Skapade {created_count} StampImage-objekt med manuella koordinater")

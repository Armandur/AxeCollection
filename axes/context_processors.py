...
def settings_processor(request):
    ...
    try:
        ...
        return {
            ...
            "display_settings": {
                "axes_rows": (
                    int(settings.default_axes_rows_private)
                    if request.user.is_authenticated
                    else int(settings.default_axes_rows_public)
                ),
                "transactions_rows": (
                    int(settings.default_transactions_rows_private)
                    if request.user.is_authenticated
                    else int(settings.default_transactions_rows_public)
                ),
                "manufacturers_rows": (
                    int(settings.default_manufacturers_rows_private)
                    if request.user.is_authenticated
                    else int(settings.default_manufacturers_rows_public)
                ),
            },
            # Footer information

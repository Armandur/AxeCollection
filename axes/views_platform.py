...
def platform_detail(request, pk):
    ...
    transactions = Transaction.objects.filter(platform=platform).select_related(
    ...
    profit_loss_with_shipping = (total_sale_value + total_sale_shipping) - (total_buy_value + total_buy_shipping)
    
    # Unika yxor
    unique_axes = transactions.values('axe__manufacturer__name', 'axe__model', 'axe__id').distinct()
...
def platform_delete(request, pk):
    ...
    if request.method == 'POST':
        if transaction_count > 0:
            messages.error(request, f'Kan inte ta bort plattformen "{platform.name}" eftersom den används i {transaction_count} transaktioner.')
            return redirect('platform_detail', pk=platform.pk)
